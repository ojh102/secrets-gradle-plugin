...
class SecretsPlugin : Plugin<Project> {
    ...
    /**
     * Injects into the main variant and, additionally, the manifest placeholders into its test
     * components (unit test / android test). The test manifests are merged from the same sources
     * and reference the same placeholders, so without this AGP fails the test manifest merge with
