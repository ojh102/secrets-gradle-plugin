...
/**
 * Injects manifest placeholders into a test component (unit test / android test).
 *
 * The merged test manifest references the same placeholders (e.g. ${MAPS_API_KEY}) as the
 * main variant, but unlike [Variant], test components only expose [manifestPlaceholders]
 * (they do not generate a BuildConfig). Without this, AGP fails the test manifest merge with
 * "requires a placeholder substitution but no value is provided".
 */
